...
orderedcontigs <- function(aligns, sortdecreasing = TRUE) {
  ctginfo <- contiginfo(aligns) 
  orderedctginfo <- ctginfo[order(ctginfo$minrefpos, decreasing = sortdecreasing), ]
...
plotaligns <- function(aligns, index=1, suppressaxis=FALSE, suppresstitle=FALSE, suppressxunits=FALSE, chrommin=NA, chrommax=NA, querymin=NA, querymax=NA, vertline=NA, vertlinelabel=NA, refgaps=NA, querygaps=NA) {
  ctginfo <- contiginfo(aligns)
  orderedctginfo <- orderedcontigs(aligns, sortdecreasing=FALSE)
...
multiplotaligns <- function(aligns, chromlength=NA, chromplotfile=NA, minfrac=0.5, refgapfile=NA, querygapfile=NA, vertlinepos=NA, maxqueryentries=10, suppresstitle=FALSE, chrommin=NA, chrommax=NA, querymin=NA, querymax=NA, unitbases=axisunitbases, querytickdistance=10) {
  ...
  bigclustercontiglist <- unique(positionorderedqueryinfo[ , "query"])
  bigclustercontigaligns <- lapply(positionorderedqueryinfo$query, function(x) {return(aligns[aligns$query==x, ])})
  
  bigcontigalignlist <- aligns[is.element(aligns$query, bigclustercontiglist),]
  contigclusters <- unique(bigcontigalignlist$cluster)
  nonsmallcontigclusters <- contigclusters[!str_detect(contigclusters, "Small")]
  
  clusterpalette <- list()
  for (i in seq(1, length(nonsmallcontigclusters))) {
    colorindex <- 1 + (i-1) %% length(safe_colorblind_palette)
    clusterpalette <- append(clusterpalette, safe_colorblind_palette[colorindex])
  }
  lennonsmallclusters <- length(nonsmallcontigclusters)
  lenclustercolors <- length(clusterpalette)
  bigclustercolors <- data.frame("clustername"=nonsmallcontigclusters, "clustercolor"=unlist(clusterpalette))
  
  orderedclusters <- str_sort(contigclusters, numeric=TRUE)
  
  legendclustercolors <- sapply(orderedclusters, function(x) {
    ifelse(str_detect(x, "Small"), "black", bigclustercolors[which(nonsmallcontigclusters==x, arr.ind=TRUE), "clustercolor"] )
  })
  ...
  # The -1 index removes the first pseudostart from the list (since that's already got a line for the x-axis)
  abline(h=contigdividelines[-1])
  
  alltickpseudopositions = c()
  allticklabels = c()
  for(contigaligns in bigclustercontigaligns) {
    ...
    segmenty0vals <- sapply(seq(1, length(contigaligns$query)), function(x) { contigaligns[x, "querystart"] - contigmin + contigpseudostart })
    segmenty1vals <- sapply(seq(1, length(contigaligns$query)), function(x) { contigaligns[x, "queryend"] - contigmin + contigpseudostart })
    ...
    contigaxisspan <- contigpseudoend - contigpseudostart
    maxlabellength <- contigaxisspan*2
    if (length(contigname) > maxlabellength) {
      contigname <- strtrim(contigname, maxlabelwidth)
    }
