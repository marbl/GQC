...
ngaxfile=paste(c(outputdir, "/NGAx.pdf"), sep="", collapse="")
pdf(ngaxfile, width=7, height=7)
assembly_ngax_plot(scaffoldsizefiles, assemblylabels=assemblylabels, ideal=TRUE, idealname=benchname, aung=FALSE, plottitle="NGAx for different assemblies", idealfile=idealfile)
dev.off()
subsfile=paste(c(outputdir, "/SubstitutionRates.pdf"), sep="", collapse="")
pdf(subsfile, width=7, height=7)
assembly_substitutions_plot(substitutionstatsfiles, assemblylabels=assemblylabels, legendypos=150, titleval="Substitution discrepancies in assemblies")
dev.off()
indelfile=paste(c(outputdir, "/IndelRates.pdf"), sep="", collapse="")
pdf(indelfile, width=7, height=7)
assembly_indels_plot(indelstatsfiles, shortassemblylabels, titleval="Indel discrepancy rates in assemblies", legendypos=20.0)
dev.off()
homopolymerfile=paste(c(outputdir, "/HomopolymerRuns.pdf"), sep="", collapse="")
pdf(homopolymerfile, width=7, height=7)
assembly_mononucqv_plot(mnstatsfiles, assemblylabels, errorbars=TRUE, pointcex=0, plotlines=TRUE, linetype=2)
dev.off()

summaryfile=paste(c(outputdir, "/SummaryStatsPlots.pdf"), sep="", collapse="")
pdf(summaryfile, width=10, height=10)
par(mfrow=c(2,2))
assembly_ngax_plot(scaffoldsizefiles, assemblylabels=assemblylabels, ideal=TRUE, idealname=benchname, aung=FALSE, plottitle="NGAx for different assemblies", idealfile=idealfile)
assembly_substitutions_plot(substitutionstatsfiles, assemblylabels=assemblylabels, legendypos=150, titleval="Substitution discrepancies in assemblies")
assembly_indels_plot(indelstatsfiles, shortassemblylabels, titleval="Indel discrepancy rates in assemblies", legendypos=20.0)
